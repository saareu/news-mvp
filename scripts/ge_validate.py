...
Validates CSV files generated by the news ETL pipeline for data quality and integrity.
Performs checks for required columns, data types, uniqueness, and URL validity.
...
Validates:
    - Required columns: id, title, pubDate (link optional)
    - Non-null values in required columns
    - Unique IDs
    - Valid URLs in link column (if present)
