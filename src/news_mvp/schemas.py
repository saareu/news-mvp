...
class Stage(str, Enum):
    ...
    ETL_AFTER_MERGE = "etl_stage_after_merge"


# For etl before merge schema (etl_stage_before_merge):
MASTER_SCHEMA_ETL_BEFORE_MERGE: List[ColumnSpec] = [
    ...
    ColumnSpec(
        "fetching_time", False, "str", "When the feed/item was fetched"
    ),  # keep last, add items before that one
]


# For etl after merge schema (etl_stage_after_merge):
MASTER_SCHEMA_ETL_AFTER_MERGE: List[ColumnSpec] = [
    ColumnSpec(
        "article_id", False, "str", "Deterministic ID from title|pub_date|source"
    ),
    ColumnSpec("guid", False, "str", "Source GUID/URL"),
    ...
    ColumnSpec("merge_time", False, "str", "When the articles were merged"),
]

_SCHEMA_REGISTRY: Dict[Stage, List[ColumnSpec]] = {
    Stage.ETL_BEFORE_MERGE: MASTER_SCHEMA_ETL_BEFORE_MERGE,
    Stage.ETL_AFTER_MERGE: MASTER_SCHEMA_ETL_AFTER_MERGE,
}


# General schema access methods, for all stages:
